<div class="category-list clearfix">
    ...
    <h1>Все рубрики в категории "<?=$cat->title?>"</h1>
    <?php foreach($categories as $category){
    $ch = Category::modelWhere('id_parent = ?', array($category->id));
        ...
        <div class="col-md-4">
            <div class="category">
                ...
                <div class="links-holder">
                    ...
                    <a href="/cp/category/grandchildren/<?=$category->id; ?>" class="btn btn-default lots">Подкатегории</a>
                    <a href="/cp/category/add/<?=$category->id; ?>" class="btn btn-default edit">Редактировать</a>
                    <a href="#" class="btn <?=(!$ch&&!$lots)?'btn-danger del-categ':'btn-default nodel'?>" data-id="<?=$category->id; ?>" onclick="return false">Удалить</a>
                </div>
            </div>
        </div>
    <?php 
